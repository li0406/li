...
func NewSmsCollect() *smsCollector {
    opts := prometheus.CounterOpts{Namespace: "qzsms", Subsystem: "http", Name: "sms_send_count", Help: "sms_send_count"}
    return &smsCollector{
        reqDesc: prometheus.NewCounterVec(opts, []string{"action","module","type","gateway","template_id"}),
        respsizeDesc: prometheus.NewDesc("qzsms_http_respsize_count", "http_respsize_count", nil, nil),
    }
}
