...
if (!function_exists('ip2region')) {
     ...
     * 根据IP获取区域
     * @access public
     * @param mixed $ip ip地址 多个ip以数组传入
     * @param string $mode 支持三种算法模式 btree|memory|binary
     * @return mixed
     */
    function ip2region($ip, $mode = 'btree')
    {
        if(empty($ip)){
            return [];
        }
        ...
        if(is_array($ip)){
            foreach ($ip as $k=>$v) {
                if($mode == 'btree'){
                    $info = $ip2region->btreeSearch($v);
                }else if($mode == 'memory'){
                    $info = $ip2region->memorySearch($v);
                }else if($mode == 'binary'){
                    $info = $ip2region->binarySearch($v);
                }       
                $area[$v][] = $info ? explode('|', $info['region']) : [];
            }
        }else{
            if($mode == 'btree'){
