$(function(){
    initCity(city_id);
    function initCity(city_id){
        App.citys.init(".money_cs", ".money_qy",shen,shi,city_id);
    }
    //图片列表免费设计
    $(document).on('click','.btn-sheji',function(){
        $('input[name=iwant_name]').val('');
        $('input[name=iwant_tel]').val('');
    ...
    $(".iwantzx-btn-1").click(function(event) {
        ...
        if (city == '') {
            // alert('您还没有选择城市哦 ^_^!');
            $("select[name=iwant_cs]").siblings('.valdate-info').html('您还没有选择城市哦 ^_^!');
            $("select[name=iwant_cs]", container).addClass('focus').focus();
            return false;
        }
        if (!App.validate.run(name)) {
            // alert('请输入您的称呼 ^_^!');
            $("input[name=iwant_name]").siblings('.valdate-info').html('请输入您的称呼 ^_^!');
            $("input[name=iwant_name]").addClass('focus').focus();
            return false;
        } else {
            var reg = new RegExp("^[\u4e00-\u9fa5a-zA-Z]+$");
            if (!reg.test($("input[name=iwant_name]", container).val())) {
                // alert("请输入正确的名称，只支持中文和英文");
                $("input[name=iwant_name]").siblings('.valdate-info').html('请输入正确的名称，只支持中文和英文');
        ...
        if (!App.validate.run(tel)) {
            // alert("请输入正确的手机号码 ^_^!");
            $("input[name=iwant_tel]").siblings('.valdate-info').html('请输入正确的手机号码 ^_^!');
