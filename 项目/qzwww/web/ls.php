 ...
 * @return mixed
 */
function get_client_ip($type = 0,$custom = '') {
    $type       =  $type ? 1 : 0;
    static $ip  =   NULL;
    if ($ip !== NULL) return $ip[$type];
    if($custom){
        //IP获取函数get_client_ip()取值顺序
        //样例 HTTP_CF_CONNECTING_IP,HTTP_X_FORWARDED_FOR,HTTP_X_REAL_IP,HTTP_CLIENT_IP,REMOTE_ADDR
        if (!empty($custom)) {
            $GET_IP_VARS_ORDER_ARR = explode(',', $custom);
        }
...
<table>
    ...
    <tbody>
        <tr>
            <td width="15%">获取IP,SERVER变量顺序</td>
            <td width="60%">
              <input type="text" name="ipOrder" size="50" placeholder="" value="HTTP_CF_CONNECTING_IP,HTTP_X_FORWARDED_FOR,HTTP_X_REAL_IP,HTTP_CLIENT_IP,REMOTE_ADDR"
              style="width: 540px;" />
            </td>
            <td width="25%">
            <input class="btn" type="submit" id="chkipOrder" value="查看IP" />
            </td>
        <script language="javascript" type="text/javascript">
            $("#chkipOrder").click(function(event) {
                window.location = '?iporder=' + $("[name='ipOrder']").val();
            });
        </script>
        </tr>
        <tr>
        ...
        <?php
            ...
            $ipOrder = trim($_GET['iporder']);
            $ipOrder = strtoupper($ipOrder);
        ?>
          <td><?php echo get_client_ip(0,$ipOrder)?></td>
...
<table>
  ...
  <tr>
    ...
    <?php echo (false!==preg_match("phpinfo/i",$disFuns))? '<font color="red">×</font>' :"<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>";?>
