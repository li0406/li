...
class WeiboOAuth2 extends BaseClient{
	...
	protected function _tokenFilter($response){
		...
		if (!is_array($token) || isset($token['error']) ) {
			...
			//throw new \Think\Exception("get access token failed." . $token['error']);
		}
	 ...
	 */
	public function getUserShow($url, $method, $headers = array()){
		$response = $this->http($url,$method,null,$headers);
		return $this->_tokenFilter($response);
	}

	public function getShortUrl($url = "", $method = "",$postfields, $headers = array())
    {
