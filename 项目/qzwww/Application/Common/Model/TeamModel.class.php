...
class TeamModel extends Model{
    ...
    public function getInviteCompanyCount($id){
        $map = array(
                "userid"=>array("EQ",$id),
                "zt"=>array("NEQ",2)//状态为未入住状态 0 未处理 1 拒绝
                     );
        return M("team")->where($map)->count();
    }
    ...
    {
        $map = [];
        $map['team.userid'] = array('NEQ',$userid);
        $map['team.comid'] = $company_id;
        $map['team.zt'] = 2;
        //作品案例的条件
//        $map['cases.isdelete'] = 1;
//        $map['cases.uid'] = $company_id;
//        $map['cases.status'] = 2;
//        $map['cases.classid'] = array('lt',3);
        return M('team')->alias('team')
            ->where($map)
            ->field('user.id,user.logo,user.name,team.zw,user_des.jobtime,user_des.fengge,count(cases.id) countcases')
            ->join('INNER JOIN qz_user as user on user.id = team.userid')
            ->join('INNER JOIN qz_user_des as user_des on user_des.userid = team.userid')
            ->join('LEFT JOIN qz_cases as cases on cases.userid = team.userid AND cases.isdelete = 1 AND cases.uid = team.comid AND cases.`status` = 2 AND cases.classid < 3')
            ->limit(4)
            ->order('team.px desc,team.id asc')
            ->group('team.userid')
            ->select();
