...
class TuLogicModel
{
    ...
    private function transformListData(array $data)
    {
        ...
        //全屋定制
        if ($formattedData['type'] == 2 && $formattedData['is_house_custom'] == 1) {
    ...
    public function getPubList(array $params)
    {
        ...
        if ($count > 0) {
            ...
            $extra = [
                'baseURL' => $schemeHost["scheme_full"].C('QZ_YUMINGWWW').'/tu/' . $params['original'] . '/',
                'params'=>$params,
            ];
            $page = new \LitePage($params['p'], $this->perCount, $count, $config, $extra);
            $show = $page->tuShow();
            $data = $this->tuModel->getPubList($params, ($params['p'] - 1) * $this->perCount, $this->perCount);
            $formattedData = [];
            if (!empty($data)) {
    ...
    public function getHomeList(array $params)
    {
        ...
        if ($count > 0) {
            ...
            $page = new \LitePage($params['p'], $this->perCount, $count, $config, $extra);
            $show = $page->tuShow();
            $data = $this->tuModel->getHomeList($params, ($params['p'] - 1) * $this->perCount, $this->perCount);
            $formattedData = [];
            if (!empty($data)) {
                foreach ($data as $value) {
                    $formattedData[] = $this->transformListData($value);
     ...
     * @return array
     */
    public function getHouseCustom(array $params)
    {
        $params['is_house_custom'] = 1;
        $params['type'] = 2;
        $count = $this->tuModel->getHomeCount($params);
     ...
     * @return mixed
     */
    public function getHouseCustomHome($page, $perCount)
    {
        $params['type'] = 2;
        $params['is_house_custom'] = 1;
        $data = $this->tuModel->getHouseCustomHome($params, ($page - 1) * $perCount, $perCount);
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                $data[$key]['image_src'] = $value['image_src'] ? $this->transformImage($value['image_src']) : '';
                $data[$key]['image_title'] = $this->decode($value['image_title']);
                $data[$key]['title'] = $this->decode($value['title']);
            }
     ...
     * @throws Exception
     */
    public function getHouseCustomHomeAjax($params)
    {
        $params['type'] = 2;
        $params['is_house_custom'] = 1;
        $data = $this->tuModel->getHouseCustomHome($params, ($params['p'] - 1) * $params['perCount'], $params['perCount']);
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                $data[$key]['image_src'] = $value['image_src'] ? $this->transformImage($value['image_src']) : '';
                $data[$key]['image_title'] = $this->decode($value['image_title']);
    ...
    public function getDetail($id, $type)
    {
        ...
        if ($type == 1) {
            ...
            $data = $this->tuModel->getHouseCustomDetail($id);
        } else {
...
//            $filterParams['parent_name'] = $this->tuCategoryLogic->value($categoryData['parent'],'name');
    ...
    {
        if ($meitu_type == 1) {
            $meituinfo = $this->tuModel->getPubDetail($id);
        } else {
            $meituinfo = $this->tuModel->getHomeDetail($id);
        }


        $getlist = $this->tuModel->getSingleCases($id, $type, $preNum, $meitu_type, $uid);
        if (count($getlist) > 0) {
    ...
    public function getRelevantImgsByImgId($id)
    {
        ...
        foreach ($result as $key => $value) {
            ...
            if($value['image_src']){
                $result[$key]['image_src'] = C('QINIU_SCHEME')."://".C('QINIU_DOMAIN')."/" . $value['image_src']."-w300.jpg";
            }
        }
        $return = [];
    ...
    {
        $data = $this->tuModel->getHouseCustomDetailOther($id, $count);
        if (!empty($data)) {
            foreach ($data as $key => $value) {
     ...
     * @return array
     */
    public function getRelativeTu($id, $count)
    {
        //获取当前三级分类
        $detail = $this->tuModel->getHouseCustomDetail($id);
        $map = [
            'home_space_category' => $detail['home_space_category'],
            'home_part_category' => $detail['home_part_category'],
            'home_style_category' => $detail['home_style_category'],
//            'home_layout_category' => $detail['home_layout_category'],
        ];
        $data = $this->tuModel->getHouseCustomRelative($id, $map, $count);
        ...
        if ($leftCount > 0) {
            foreach ($map as $key => $value) {
                ...
                $leftData = $this->tuModel->getHouseCustomRelative($id, $newMap, $leftCount);

                //查看是否有重复的数据
                //去重
                $idsData = array_column($data, 'id');
                foreach ($leftData as $v) {
                    if(!in_array($v['id'],$idsData,true)){
                        array_push($data, $v);
                    }
                }
                $leftCount = $count - count($data);
        ...
        foreach ($data as $key => $value) {
            $data[$key]['publish_time'] = date('Y-m-d H:i:s', $value['publish_time']);
            $data[$key]['image_src_total'] = $this->transformImage($value['image_src']);
        }
        return $data;
    }
}
