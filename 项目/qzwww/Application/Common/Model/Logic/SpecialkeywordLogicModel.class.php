...
class SpecialkeywordLogicModel
{
    ...
    public function getActriceBySpecialkeywordList($keyword_module,$name,$tempword, $pageIndex = 0, $pageCount = 12,$notid = 0){
        ...
        return $model->getActriceBySpecialkeywordList($keyword_module,$name,$tempword,$pageIndex,$pageCount,$notid);
    }
    ...
    public function getActriceBySpecialkeywordCount($keyword_module,$name, $tempword,$notid = 0){
        ...
        return $model->getActriceBySpecialkeywordCount($keyword_module,$name,$tempword,$notid);
    }
    /**
     * [getSpecialkeywordList 获取关键字列表]
     * @param  [type] $keyword   [搜索关键字]
    ...
    public function getSpecialkeywordByAbout($keyword_module,$name,$tempword){
        if(!empty($name) && count($tempword)>0){
            return D('Common/Db/Specialkeyword')->getSpecialkeywordByAbout($keyword_module,$name,$tempword);
        }
        return false;
    }
    public function getSpecialkeywordByAboutv2($keyword_module,$name,$tempword){
        if(!empty($name) && count($tempword)>0){
            return D('Common/Db/Specialkeyword')->getSpecialkeywordByAboutv2($keyword_module,$name,$tempword);
        }
        return false;
    }
    
    public function getLinkWordsByAboutWords($keyModule,$linkWords)
    {
	    if(count($linkWords)>0){
		    return D('Common/Db/Specialkeyword')->getLinkWordsByAboutWords($keyModule,$linkWords);
	    }
	    return false;
    }
     ...
     * @return [type]            [description]
     */
    public function getNewSpecialkeyword($keyword_module,$name,$time,$id,$limit = 20){
        if (!empty($keyword_module)) {
            $map['keyword_module'] = $keyword_module;
        }
        if (!empty($time)) {
        ...
        if (!empty($name)) {
            $map['name'] = array('NEQ',$name);
        }
        return D('Common/Db/Specialkeyword')->getNewSpecialkeyword($map,$limit);
    }


    public function getTitleKeywords($title,$keyword_module)
    {
        //调用分词接口
        $data['word'] = $title;
        $result = curl(C('FENCI_DONAMES'), $data, 0);
        //获取最新三条关键词
        if (!empty($result)) {
    ...
    public function getKeywords(){
        ...
        return $keyword_model->getSpecialkeywordList($map,0, 20,'distinct name,href');
    }

    public function getLastKeyword($id){
		$keyword_model = new SpecialkeywordModel();
		$map['keyword_module'] = 3;
		$map['id'] = array('lt',$id);
		return $keyword_model->getNewSpecialkeyword($map);
	}
...
//            $map['name'] = array('in',$data);
    ...
    {
        $model = new SpecialkeywordModel();
        $gongluelist = $model->getGongLveZhuanTiList(1,$limit); //专题栏目，1：装修攻略
//        dump($gongluelist);die;
        foreach ($gongluelist as $key => $val){
            if($val['article_face']){
                $gongluelist[$key]['article_face'] = changeImgUrl($val['article_face']);
            }
            $gongluelist[$key]['zhuanti_name'] = mbstr($val['zhuanti_name'],0,10,"UTF-8");
        }
        return $gongluelist;
    }
    ...
    {
        $model = new SpecialkeywordModel();
        $baikelist = $model->getBaikeZhuanTiList(2,$limit); //专题栏目，：装修百科
        foreach ($baikelist as $key => $val){
            ...
            $baikelist[$key]['article_description'] = mbstr($val['article_description'],0,50,"UTF-8");
        }
        return $baikelist;
    }
    ...
    public function getWendaZhuanTiList($limit)
    {
        ...
        foreach ($wendalist as $key => $val){
            $wendalist[$key]['name'] = mbstr($val['name'],0,10,"UTF-8");
        }
        return $wendalist;
    }
