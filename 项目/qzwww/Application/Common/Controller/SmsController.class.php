...
class SmsController extends Controller {
    ...
    {
        $data = $_POST;
        $returnData = isset($_POST['return_data']) ? $_POST['return_data'] : '';
        if (!empty($data['code'])) {
            ...
            if (!empty($w_ordersafecode)) {
                ...
                if ($tel != $arr['tel']) {
                    $this->ajaxReturn(array('data' => '', 'info' => '亲,帐号不符,请输入正确的手机/邮箱', 'status' => 0));
                }
                if (strtolower($data['code']) != authcode($arr['code'])) {
                    $this->ajaxReturn(array('data' => '', 'info' => '亲,您输入的验证码不正确', 'status' => 0));
                }
                //验证通过给一个session对象用于回调方法的验证
                $_SESSION["isverify"] = 1;
                if($data["isverify"] != '2'){
                    setcookie("w_ordersafecode",null,-1,'/', '.'.APP_HTTP_DOMAIN);
                }
                $this->ajaxReturn(array('data' => $returnData, 'info' => '亲,验证码输对了！', 'status' => 1));
            } else {
                $this->ajaxReturn(array('data' => '', 'info' => '亲,验证码失效了！', 'status' => 0));
            }
        } else {
            $this->ajaxReturn(array('data' => '', 'info' => '亲,验证码不正确！', 'status' => 0));
        }
    }
