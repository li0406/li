...
class ZxinfoController extends SubBaseController
{
    public function _initialize()
    {
        ...
        //添加顶部搜索栏信息
        $this->assign('serch_uri',$this->SCHEME_HOST['scheme_full'].C('QZ_YUMINGWWW').'/gonglue/search');
    ...
    public function index()
    {
        ...
        //移动端跳转处理
        if (ismobile()) {
            header('Location: '.$this->SCHEME_HOST['scheme_full']. C('MOBILE_DONAMES') .'/'.$cityInfo['bm'].'/zxinfo/');
            exit();
        }
        $this->setUrlOver();
        //文章分类和文章内容
        ...
        //canonical标签
        $header['canonical'] = $this->SCHEME_HOST['scheme_full']. $cityInfo['bm'] . '.' . C('QZ_YUMING') . '/zxinfo/';
    ...
    public function category()
    {
        ...
        $category = I('get.category','');
            //移动端跳转处理
        if (ismobile()) {
            header('Location: '.$this->SCHEME_HOST['scheme_full']. C('MOBILE_DONAMES') .'/'.$cityInfo['bm'].'/zxinfo/'.$category.'/');
            exit();
        }
        if (is_numeric($category)){
            if (ismobile()) {
                header('Location: '.$this->SCHEME_HOST['scheme_full']. C('MOBILE_DONAMES') .'/'.$cityInfo['bm'].'/zxinfo/'.$category.'.html');
                exit();
            } else {
                $url = $this->SCHEME_HOST['scheme_full']. $cityInfo['bm'] . '.' . C('QZ_YUMING') . '/zxinfo/'.$category.'.html';
                header('Location: $url' );
                exit();
            }
        }
        ...
        //页面canonical
        $header['canonical'] = $canonical = $this->SCHEME_HOST['scheme_full'].$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        if($articleList['nowpage'] > 1){
            $header['canonical'] = str_replace('-'.$articleList['nowpage'],'',$canonical);
        }

        //[地域]公司活跃榜
        $activitycompanylist = S('Cache:SubCompany:NotVipActivityCompany2'.$this->cityInfo['bm']);
        ...
        if(!$activitycompanylist['time']){
            ...
            S('Cache:SubCompany:NotVipActivityCompany2'.$this->cityInfo['bm'],$savecache,43200);
        }else{
        ...
        $this->assign('activitycompanylist',$activitycompanylist['list']);


        $this->assign('cityname',$cityInfo['cname']);
    ...
    public function article()
    {
        ...
        //移动端跳转处理
        if (ismobile()) {
            exit(header('Location: ' . $this->SCHEME_HOST['scheme_full']. C('MOBILE_DONAMES') . '/' . $this->cityInfo['bm'] . '/zxinfo/' . $id . '.html'));
        }
        ...
        if (empty($articleInfo)) {
            ...
            //相关文章
            S('Cache:SubArticle:Littlearticle:' . $id, $articleInfo, 60);
        }
        ...
        $keywords = $thematicLogic->getContentLabels($id,4,5);

        //大家都在问
        $articleLogic = new ArticleLogicModel();
        if(count($keywords) > 0){
            $info['ask'] = S('Cache:Zxinfo:Index:EveryoneAsk');
            if (!$info['ask']) {
                $ask = $articleLogic->getAskListByKeywords($keywords);
                $info['ask'] = $ask;
                S('Cache:Article:Zxinfo:EveryoneAsk', $ask, 900);
            }
        }
        ...
        $info['head']['canonical'] = $this->SCHEME_HOST['scheme_full']. $this->cityInfo['bm'] . '.' . C('QZ_YUMING') . '/zxinfo/' . $id . '.html';
        ...
        $this->assign('cityname',$this->cityInfo['cname']);
        ...
        if(empty($articleInfo['article']['now']['optime'])){
            $optimee =date("Y-m-d",$articleInfo["article"]['now']['addtime']);
            $optimes =date("H:i:s",$articleInfo["article"]['now']['addtime']);
        }else{
            $optimee =date("Y-m-d",$articleInfo["article"]['now']['optime']);
            $optimes =date("H:i:s",$articleInfo["article"]['now']['optime']);
        }
        $baidu['optime'] = $optimee."T".$optimes;
        $baidu['addtime'] = date("Y-m-d",$articleInfo["article"]['now']['addtime'])."T".date("H:i:s",$articleInfo["article"]['now']['addtime']);
        $this->assign("baidu",$baidu);

        //获取文章关联分站专题
        $logic = new SubthematicLogicModel();
//        $zhuanti = $logic->getArticleListByTag($articleInfo['article']['now']['tags'],$this->cityInfo['id']);
        $zhuanti = $logic->getArticleListBySubTag($id,$this->cityInfo['id']);
    ...
    public function remoteArticle()
    {
        ...
        if ($id > 0) {
            ...
            exit(header('Location:' .$this->SCHEME_HOST['scheme_full']. $this->cityInfo['bm'] . '.' . C('QZ_YUMING') . '/zxinfo/' . $id . '.html'));
        } else {
            exit($this->_error());
        }
    }
                ...
                foreach ($matches[0] as $key => $value) {
                    ...
                    foreach ($m[1] as $j => $v) {
                        ...
                        if (strpos($v, '-s5.')) {
                            ...
                            $article['now']["content"] = str_replace($v, $path, $article['now']["content"]);
                        }else{
                            $path .= '-s3.jpg';
                            $article['now']["content"] = str_replace($v, $path, $article['now']["content"]);
                        }
