  ...
  <div class="order-content">
    <div class="search-box">
      ...
      <div class='content-form'>
        <el-form :inline="true" ref="formInline" label-width="80px" :model="formInline">
          <div>
            ...
            <el-form-item label="预算" prop="yusuan" class="yusuan">
              ...
              >
                ...
                <el-option v-for="item in yusuan"
                           :label="item.name"
                           :value="item.id"
                           :key="item.id"
                />
          ...
          <div>
            ...
            <el-form-item label="选择面积" prop="mianji">
              <el-select
                v-model="formInline.mianji"
                placeholder="选面积"
              >
                <el-option value="0" label="请选择"/>
                <el-option
                  v-for="item in mianji"
                  :key="item.id"
                  :value="item.id"
                  :label="item.name"
                />
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <p>{{ scope.row.first_allot_date}}</p>
            </template>
          </el-table-column>
          <el-table-column
            prop="yz_name"
          ...
          <el-table-column
            ...
            label="是否量房">
            <template slot-scope="scope">
              <p>{{ scope.row.liangfang_status === 1?"已量房" : (scope.row.liangfang_status === 2?"未量房":"未知")}}</p>
            </template>
          </el-table-column>
          <el-table-column
            prop="operate"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.call_repeat_count>0"
                         type="text" size="small"
                         @click="recordHandle(scope.row.id)">呼叫记录({{scope.row.call_repeat_count}})
              </el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <qz-dialog :dia-title="'订单编号：' + orderInfo.id" :dialog-visible="dialogTableVisible" :dia-width="'650px'"
      ...
      <table v-loading="dialoading" class="dia_table " :class="{'isqiandan':qiandan_status=='1'}">
        ...
        <tr>
          <td width="80">选择公司：</td>
          <td colspan="3">
            <span v-for="item in orderInfo.companys" class="mr10" :class="{red:item.isread}">{{item.jc}}</span>
          </td>
        </tr>
      </table>
    </qz-dialog>
