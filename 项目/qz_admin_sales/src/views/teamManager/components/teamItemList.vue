<template>
  <div class="team-item-list">
    ...
    <el-table v-loading="loading" :data="tableData" style="width: 100%">
      <el-table-column prop="team_name" label="团队名称/月份" />
      ...
      <el-table-column prop="address" label="操作">
        <template slot-scope="scope">
          ...
          >编辑</span>
          <span v-if="scope.row.isSet" @click="handleDel(scope.row)">取消</span>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { getTeamList, fetchIndicatorsEdit } from '@/api/teamManager'
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 20,
      loading: false,
      importUrl: this.$qzconfig.base_api + '/v1/indicators/upteamindicators',
      fileList: [],
      upName: 'file',
      uploadContentType: {
  ...
  methods: {
    getList() {
      ...
      if (that.formData.name) {
        query.team_name = that.formData.name
      }
    ...
    handleTplDownload() {
      const urls = this.$qzconfig.oss_img_host + 'custom/20200429/Fq7j6SYAH_mNMJJ5D1Zz5Og68Ayu.xlsx'
