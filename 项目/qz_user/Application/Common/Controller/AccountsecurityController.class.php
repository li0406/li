...
class AccountsecurityController extends Controller{
    ...
    public function confirmAccount()
    {
        ...
        if (strtolower($code) != strtolower(authcode($arr["code"]))) {
            $this->ajaxReturn(array("data"=>"","info"=>"请输入正确的验证码！","status"=>0));
        }
        setcookie("w_ordersafecode",1,time()-1,'/', '.'.APP_HTTP_DOMAIN);
    ...
    public function account(){
        ...
        if ($_SESSION["u_userInfo"]["classid"] == 2) {
            $info["to_email"] = "/designer/sendemail";
            $info["to_sms"] = "/designer/sendsms/";
        } else {
            $info["to_email"] = "/company/sendemail";
            $info["to_sms"] = "/orders/sendsms/";
        }
