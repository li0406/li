...
class MobilePage {
    ...
    public function show(){
        ...
        if($this->pageRowCount > 0){
            ...
            $config = array(
                "prev"=>str_replace("%LINK%",(C('QZ_YUMING_SCHEME')."://".$pageLink."p=".(($this->pageIndex-1)<=1?1:($this->pageIndex-1))).(empty($query_string)?"":"?".$query_string),$this->config["prev"]),
                "link_page"=>$buttons,
                "next"=>str_replace("%LINK%",(C('QZ_YUMING_SCHEME')."://".$pageLink."p=".(($this->pageIndex+1)>=$this->pageTotalCount?$this->pageTotalCount:($this->pageIndex+1))),$this->config["next"]),
