...
use Common\Model\Db\YgjAccountLockModel;
...
class OrdersController extends CompanyBaseController
{
        ...
        if (IS_POST) {
            ...
            if (count($log) > 0) {
                if ($log["is_lock"] == 1 && $log["unlock_time"] > time()) {
                    $this->ajaxReturn([
                        "data" => "",
                        "info" => "订单查看功能被锁中,预计可看订单时间为" . date("Y-m-d H:i:s", $log["unlock_time"]),
                        'status' => 2
                    ]);
                }
            }
            ...
            if (!empty($orderPass)) {
                if ($orderPass["pass"] == md5("123456")) {
                    $this->ajaxReturn(array("info" => "为了您的订单安全，请先修改查单密码后，再操作", "status" => 3));
                }
            ...
            if (!empty($user["orderpass"])) {
                ...
                // 是否可以导出
                $info['orderInfo']["is_can_export"] = 0;
                ...
                if (I("get.state") !== "") {
                    $state = I("get.state");
                    $info['orderInfo']["state"] = $state;
                }

                if (I("get.keyword") !== "") {
                    $text = I("get.keyword");
                    $info['orderInfo']["keyword"] = $text;
                    $info['orderInfo']["is_can_export"] = 1;
                }

                if (I("get.begin") != "" && I("get.end") != "") {
                    $begin = I("get.begin");
                    $info['orderInfo']["begin"] = $begin;

                    $end = I("get.end");
                    $info['orderInfo']["end"] = $end;

                    $info['orderInfo']["is_can_export"] = 1;
                }

                $orders = $this->getOrders($user["id"], $text, $isread, $state, $begin, $end, $pageIndex, $pageCount);
    ...
    public function ordertrack(){
        ...
        if (!empty($order_id)) {
            ...
            $info["track_list"] = $companyTrackLogic->getAllByUnique($user["id"], $order_id);
            $last_track = $companyTrackLogic->getLastTrack($user["id"], $order_id);
            $info["last_track_id"] = $last_track["id"];

            $companyVisitLogic = new CompanyVisitLogicModel();
            $info["visit_list"] = $companyVisitLogic->getAllByUnique($user["id"], $order_id);

            if (count($info["visit_list"]) > 0) {
                $visitIds = array_column($info["visit_list"], "id");
                $companyVisitLogic->setCompanyVisitRead($user["id"], $visitIds);
            }

            $this->assign("info", $info);
            $this->assign("order_id", $order_id);
            $this->assign("track_step_list", $companyTrackLogic->trackStep);
            $data = $this->fetch("ordertrack");

            $this->ajaxReturn(["error_code" => 0, "error_msg" => "ok", "data" => $data]);
        }

        $this->ajaxReturn(["error_code" => 4000002, "error_msg" => "订单ID参数错误"]);
    }

    // 跟单小计添加/编辑
    public function orderTrackSave(){
        $user = session("u_userInfo");

        $order_id = I("post.order_id");
        $track_date = I("post.track_date");
        $track_step = I("post.track_step");
        $track_content = I("post.track_content");

        if (empty($order_id)) {
            $this->ajaxReturn(["error_code" => 4000002, "error_msg" => "订单ID参数错误"]);
        } else if (empty($track_date)) {
            $this->ajaxReturn(["error_code" => 4000002, "error_msg" => "请选择跟进时间"]);
        } else if (empty($track_step)) {
            $this->ajaxReturn(["error_code" => 4000002, "error_msg" => "请选择跟进阶段"]);
        } else if (empty($track_content)) {
            $this->ajaxReturn(["error_code" => 4000002, "error_msg" => "请填写跟进内容"]);
        } else if (strtotime($track_date) == false) {
            $this->ajaxReturn(["error_code" => 4000002, "error_msg" => "跟进时间格式错误"]);
        }

        $companyTrackLogic = new CompanyTrackLogicModel();
        $response = $companyTrackLogic->saveTrack(I("post.id"), [
            "company_id" => $user["id"],
            "order_id" => $order_id,
            "track_step" => $track_step,
            "track_content" => $track_content,
            "track_time" => strtotime($track_date),
        ]);

        $this->ajaxReturn($response);

        if ($result == false) {
            $this->ajaxReturn(["error_code" => 5000002, "error_msg" => "添加失败"]);
        }

        $this->ajaxReturn(["error_code" => 0, "error_msg" => "添加成功"]);
    }

    // 跟单小计删除
    public function orderTrackDelete(){
        $user = session("u_userInfo");
        $id = I("post.id");

        $companyTrackLogic = new CompanyTrackLogicModel();
        $response = $companyTrackLogic->deleteTrack($id, $user);
     ...
     */
    public function orderNewNums(){
        $user = session('u_userInfo');
        $timestamp = I("post.timestamp");

        $nowtime = time();
        if (empty($timestamp)) {
            $timestamp = $nowtime - 30;
        }
        
        // 查询分/赠单数量（使用区间查询减少误差）
        $number = D("Orders")->getNewNumbersByComid($user["id"], $timestamp, $nowtime);
    ...
    public function orderchange()
    {
        if ($_POST) {
            ...
            $result = check_pwd(I("post.password"));
            if (!$result) {
                $this->ajaxReturn(array("data"=>"","info"=>"亲,您输入的密码强度过低","status"=>0));
            }
            ...
            if ($orderpass->create($data)) {
                ...
                preg_match($reg,$data["pass"],$m);
                if (count($m) == 0) {
                    $this->ajaxReturn(array("data"=>"","info"=>"请不要填写纯数字/纯字母","status"=>0));
                }


                $data["pass"] = md5($data["pass"]);
                //查询该公司的原查询密码,新密码不能和旧密码一致
                $pass = $orderpass->getOrderPassById(session('u_userInfo.id'));
                if (count($pass) > 0) {
                ...
                $user = D("User")->getSingleUserInfoById(session('u_userInfo.id'));

                if ($user["pass"] == $data["pass"]) {
                    $this->ajaxReturn(array("data" => "", "info" => "订单查看密码不能和登录密码一致", "status" => 0));
                }

                $i = $orderpass->editPass(session('u_userInfo.id'), $data);
                ...
                if ($i !== false) {
                    ...
                    $model->updateAccount($user["id"],$data);
    ...
    public function changestate()
    {
        if ($_POST) {
            ...
            M()->startTrans();
            try {
                $company_review_logic_obj = new \Common\Model\Logic\OrderCompanyReviewLogicModel();
                ...
                if (!empty($info)) {
                    $company_review_logic_obj->editReview($post_data['id'], session("u_userInfo.id"), $data);
                } else {
                    $data["comid"] = session("u_userInfo.id");
                    $data["orderid"] = $post_data['id'];
                    $company_review_logic_obj->addReview($data);
                }
    ...
    public function changeStateComplete()
    {
        ...
        try {
            ...
            if ($data['state'] == OrderStatus::HAS_SIGN) {
                //查询该订单是否已被申请
                $orderInfo = D("Orders")->getOrderInfoById($data['id']);
                if (empty($orderInfo)) {
                    $this->ajaxReturn(['status' => ApiConfig::ORDER_MISS, 'info' => '未找到该订单']);
                }
                if (!empty($orderInfo["qiandan_companyid"])) {
                    $this->ajaxReturn(['status' => ApiConfig::ORDER_HAS_SIGN, 'info' => '订单已被签约']);
                }
                if (strtotime("2013-10-01") > $orderInfo["time"]) {
                    $this->ajaxReturn(['status' => ApiConfig::ORDER_EXPIRE, 'info' => '该订单时间太久远了,已无法签单']);
                }
                //签单金额逻辑
                $sign_data = [
            ...
            // 如果装修公司选择已量房，增加/修改：回访订单记录
            if ($data['state'] == OrderStatus::HAS_LIANG_FANG) {
                \Common\Model\Logic\OrderReviewLogicModel::saveOrderReview($data['id']);
            }

            if ($company_review_logic_obj->replaceReview($data, $user) == false) {
                $this->ajaxReturn(['status' => ApiConfig::REQUEST_FAILL, 'info' => '修改订单状态失败']);
            }
            $log_id = $company_review_logic_obj->OrderChangeLog($data['id'], $user, $data['state'], 1);

            //添加装修公司审核订单日志
            $company_review_logic_obj->saveCompanyOrderChangeLog($data['id'], $user, $data['state'],$log_id);
            // 生成或删除工地直播
            $worksiteLogic = new WorksiteLogicModel();
            if ($data['state'] == OrderStatus::HAS_SIGN) {
                $res = $worksiteLogic->createdLive(1, $data['id']);
            } else {
                $worksiteLogic->deleteLive(1, $data['id']);
            }

            if ($data['state'] != OrderStatus::NOT_LIANG_FANG) {
                //解除绑定号码保护
                $service = new PnpServiceModel();
                $service->unBindTel($data['id']);
            }
            M()->commit();
            $this->ajaxReturn(array("status" => 1, 'info' => '操作成功'));
        } catch (\Exception $e){
    ...
    public function wechat()
    {
        ...
        //查询该公司已绑定的微信帐号
        //查询公司微信绑定的信息
        $wechatList = D("Orderwechar")->getOrderNoticeList(session('u_userInfo.id'));
        if (!empty($wechatList)) {
        ...
        if (!empty($result)) {
            ...
            if (!empty($order)) {
                ...
                foreach ($tempResult as $key => $value) {
                    if (session('u_userInfo.id') == $value['id']) {
                ...
                //查询是否已读过订单
                $count = D("Orders")->getOrderReadCount($id, session('u_userInfo.id'));
                if (empty($count)) {
                    //修改阅读状态
                    D('Orders')->editOrderRead($id, session('u_userInfo.id'), ['isread' => 1, 'readtime' => time()]);
                ...
                $app = new \App();
                D('Loguser')->addLog([
                    'username' => session('u_userInfo.jc'),
                ...
                if ($order['order2com_allread'] == 0) {
                    ...
                    if ($isAllRead) {
                        ...
                        D('Orders')->editOrder($order['id'], ['order2com_allread' => 1]);
                    }
                }

                // 查询是否是工地直播
                $worksite_live_id = 0;
                if ($order["qiandan_companyid"] == $info["user"]["id"]) {
                    $worksiteLogic = new WorksiteLogicModel();
                    $worksite_live_id = $worksiteLogic->getLiveIdByOrder(1, $order["id"]);
                }

                $this->assign("worksite_live_id", $worksite_live_id);
    ...
    public function viewBackOrder()
    {
        ...
        if (!empty($result)) {
            ...
            if (!empty($order)) {
                ...
                //查询是否已读过订单
                $count = $liang_fang_logic->getOrderReadCount($id, session('u_userInfo.id'));

                if ($count > 0) {
                    //修改阅读状态
                    $liang_fang_logic->editOrderRead($id, session('u_userInfo.id'));
                }
                ...
                $company_review_logic_obj->OrderChangeLog($id, session('u_userInfo'), 99, 2);
    ...
    public function unqiandanorder()
    {
        if ($_POST) {
            ...
            if (count($result) > 0) {
                if (
                    $result["qiandan_companyid"] == session('u_userInfo.id')
    ...
    public function delreview()
    {
        if ($_POST) {
            $id = I("post.id");
            //查询是否是该公司的回访记录
            $result = D("Ordercompanyreview")->getReviewCountById($id, session('u_userInfo.id'));
            if ($result > 0) {
                $i = D("Ordercompanyreview")->deleteReview($id, session('u_userInfo.id'));
                if ($i !== false) {
                    $this->ajaxReturn(array("data" => "", "info" => "", "status" => 1));
                }
    ...
    public function unapplyorder()
    {
        if ($_POST) {
            ...
            unset($result['tel8']);
            if (count($result) > 0) {
                if ($result["qiandan_companyid"] == session('u_userInfo.id')) {
                    $i = D("Orders")->editOrder($id, $data);
                    if ($i !== false) {
                        //导入扩展文件
                        import('Library.Org.Util.App');
    ...
    public function lookinitiativeorder()
    {
        if ($_POST) {
            $id = I("post.id");
            //查询自主签单信息、
            $order = D("Orderscompanyreport")->getOrderById($id, session('u_userInfo.id'));
            if (count($order) > 0) {
                $this->assign("order", $order);
    ...
    public function addinitiativeorder()
    {
        if ($_POST) {
            $data = array(
                ...
                "order_company_id" => session('u_userInfo.id'),
                "time_add" => time(),
                "mianji" => I("post.mianji"),
                "time_qd" => I("post.time_qd")
            );
            $order_id = D("Orderscompanyreport")->addOrder($data);
            if ($order_id !== false) {
                // 生成工地直播
                $worksiteLogic = new WorksiteLogicModel();
                $worksiteLogic->createdLive(2, $order_id);
    ...
    public function initiative()
    {
        ...
        $orders = $this->getInitiativeOrders(session('u_userInfo.id'), $on, $text, $pageIndex, $pageCount);
     ...
     * @return [type]        [description]
     */
    private function getOrders($comid, $text, $isread, $state, $begin, $end, $pageIndex, $pageCount)
    {
        //过滤
        //强制数字整数
        $pageIndex = intval($pageIndex) <= 0 ? 1 : intval($pageIndex);
        ...
        $count = D("Orders")->getOrderListByComidCount($comid, $text, $isread, $state, $begin, $end);
        if ($count > 0) {
            import('Library.Org.Page.Page');
            //自定义配置项
            $config = array("prev", "next");
            ...
            $offset = ($page->pageIndex - 1) * $pageCount;
            $orders = D("Orders")->getOrderListByComid($comid, $text, $isread, $state, $begin, $end, $offset, $pageCount);

            // 查询回访未读数量
            $orderIds = array_column($orders, "order");
            $companyVisitLogic = new CompanyVisitLogicModel();
            $noreadVisitList = $companyVisitLogic->getOrderNoRead($comid, $orderIds);

            foreach ($orders as $key => $item) {
                $order_id = $item["order"];
                $orders[$key]["visit_noread_count"] = 0;
                if (array_key_exists($order_id, $noreadVisitList)) {
                    $orders[$key]["visit_noread_count"] = $noreadVisitList[$order_id];
                }
            }
