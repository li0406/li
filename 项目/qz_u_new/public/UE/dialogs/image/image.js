    ...
    switch (id) {
      case 'remote':
        remoteImage = remoteImage || new RemoteImage()
              document.getElementById('remote').getElementsByTagName('input')[0].value = myUploadPic
        break
            case 'upload':
    ...
    initUploader: function() {
      ...
      uploader = _this.uploader = WebUploader.create({
        ...
        var $li = $('<li id="' + file.id + '">' +
          ...
          $info = $('<p class="error">勿超600K</p>').hide().appendTo($li),
      ...
      document.addEventListener('click', function(event) {
        ...
        if (target && target.toLowerCase() === 'closeoversizeimg') {
          ...
          _this.imageList.splice(imgIndex, 1)
          removeFile(delNode)
        }
      }, false)
      // 负责view的销毁
        ...
        switch (type) {
          ...
          case 'stopUpload':
            ...
            uploader.on('uploadBeforeSend', function(file, data, header) {
              ...
              header['token'] = editor.getOpt('token')
              if (!header['token']) {
        ...
        var $file = $('#' + file.id)
          ...
          var responseText = (ret._raw || ret),
            json = utils.str2json(responseText)
                    myUploadPic = 'https://zxsqn.qizuang.com/' + json.data.img_path
          if (json.state == 'SUCCESS') {
            _this.imageList.push(json)
                        $file.append('<span class="success"></span>')
                    } else if (json.error_code == 0) {
            _this.imageList.push({
              url: json.data.img_path
            })
                      $file.find('img').attr('_src', json.data.img_path)
            $file.append('<span class="success"></span>')
                      $file.append('<span class="close" data-hook="closeOverSizeImg">×</span>')
                    } else {
            $file.find('.error').text(json.state).show()
                      $file.append('<span class="close" data-hook="closeOverSizeImg">×</span>')
                    }
        } catch (e) {
          $file.find('.error').text(lang.errorExceedSize).show()
                }
      })
