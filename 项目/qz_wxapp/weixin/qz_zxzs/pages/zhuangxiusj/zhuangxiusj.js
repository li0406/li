...
Page({
    ...
    formSubmit: function (e) {
        ...
        if(that.data.src){
            ...
            wx.request({
              url: apiUrl + '/zxjt/submit_order/?src=xcx-zrll-zxzs',
                data: {
                    mianji: bjmj,
                    tel: tel,
                    cs: city,
          ...
          alertViewWithCancel("提示", "请勿频繁操作，请稍后再试...", function () { });
          setTimeout(function () {
            that.setData({ clickAble: teue })
          }, 5000)
        }
    },
    ...
    onLoad: function (options) {
        ...
        if (options.src) {
            ...
            app.setNewStorage('src', options.src, 300);
        } else {
            if (app.getNewStorage('src')) {
                that.setData({ src: app.getNewStorage('src') });
            } else {
              that.setData({ src: 'xcx-zrll-zxzs' });
            }
        }
    ...
    okbtn: function () {
        ...
        this.setData({ xzcity: '',isHideCity: true, selectText: selectText, prevCityAreaId: prevCityAreaId,areaId:areaId });
    },
