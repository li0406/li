// pages/delayMemberOffer/delayMemberOffer.js
import {
  memberReportDetail,
...
Page({
  ...
  getViphistoryBtn(){
    this.setData({
      showVipHistoryModal: true
    })
    this.getViphistoryList()
  },
  getViphistoryList() {
    let that = this
    let companyId = that.data.info.company_id
    wx.getStorage({
      key: 'info',
      success: function (res) {
        let token = res.data.token;
        historyQuote('/v1/company/vip_history_list', {
  ...
  goToReport(e){
    ...
    let cooperation_type = this.data.info.report_cooperation_type
    wx.navigateTo({
