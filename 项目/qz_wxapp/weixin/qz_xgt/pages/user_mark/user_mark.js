...
Page({
            ...
            fail () {
                ...
                wx.request({
                    ...
                    success (res) {
                        ...
                        let cityUrlStr = cityUrlArr[0] + ':' + cityUrlArr[1]
                        wx.request({
        ...
        if (res.from === 'button') {
            ...
            if(res.target.dataset.style=='xgt'){
                return {
                    ...
                    path: '/pages/xgt_det_single/xgt_det_single?id=' + res.target.dataset.id + '&title=' + res.target.dataset.title,
    ...
    toxgtDet (e){
        ...
        wx.navigateTo({
            url: '../xgt_det_single/xgt_det_single?id=' + id + '&title=' + title
        })
    },
    toDel (e){
        let id = e.currentTarget.dataset.id,
    ...
    toXGT(){
        let feng = app.getNewStorage('fenggeid'),
            location = app.getNewStorage('location'),
            huxing = app.getNewStorage('huxingid'),
            fenggetext = app.getNewStorage('fenggetext');
        wx.navigateTo({
            url: '../xgt_list_home/xgt_list_home?loca=' + location + '&feng=' + feng + '&Fenggetext=' + fenggetext
            
        })
    },
    downLoad(){
        let that = this,
            xgtCount=that.data.countXgt+10;
