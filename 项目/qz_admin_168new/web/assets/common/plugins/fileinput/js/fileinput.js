...
(function (factory) {
    ...
    tBtnLink = '<a href="javascript:void(0)" tabindex="500" title="{title}" class="{css}" {status}>{icon}{label}</a>';
    ...
    FileInput.prototype = {
        ...
        clear: function () {
            ...
            if (self.hasInitialPreview()) {
                ...
                self.resetPreview();
                self.initPreviewDeletes();
                // self.$container.removeClass('file-input-new'); // 注释掉了这句
                addCss(self.$container, 'file-input-new'); // 增加了这句
            } else {
                self.getThumbs().each(function () {
        ...
        resetPreview: function () {
            ...
            if (previewCache.count(self.id)) {
                out = previewCache.out(self.id);
                console.log("修改了这句：self.$preview.html(out.content)");
                // self.$preview.html(out.content);
                self.$preview.html("");
        ...
        change: function (e) {
            var self = this, $el = self.$element;
            //这段是新增的
            // if (self.$preview.find('.file-preview-frame').length >= self.options.fileNumLimit) {
            //     return false;
            // }
