...
function myUp(picCon, listCon) {
    ...
    var $filePicker = $(picCon), // 上传按钮
        ...
        initObj = {
            ...
            auto: true,
            // swf文件路径
            swf: '/assets/common/js/plugins/webuploader/js/uploader.swf',
            // 文件接收服务端。
            server: "/upload/uploadImg/",  //  这里是图片上传地址
            // 选择文件的按钮。可选。
    ...
    uploader.on('fileQueued', function (file) {
        ...
        if (tempName == "cover-img") {
            var limitSise = 4 * 1024 * 1024;
            if (file.size > limitSise) {
                layer.msg('图片大小超出上限,请从新上传');
                return false;
            }
            $li = $(
                '<div data-id="'+file.id+'" id="' + file.id + '" class="file-item thumbnail logo-img">' + '<input class="nameval" type="hidden" value=""/>' +
                '<img>' + '<span class = "cancel delimgbtns" title="删除"><i class="fa fa-close" aria-hidden="true"></i></span>' + '</div>'
                /* '<div class="info">' + '' + '</div>' +
                 '<div class = "file-panel" style = "height: 30px;" ><input class="inbiaoti" type="text" valur="" placeHolder="点击输入标题"></div>' +
                 '</div>'*/
            );
            ...
            $li = $(
                '<div data-id="'+file.id+'"  id="' + file.id + '" class="file-item thumbnail">' + '<input class="nameval" type="hidden" value=""/>' +
                '<img>' + '<span class = "cancel delimgbtns" title="删除"><i class="fa fa-close" aria-hidden="true"></i></span>' +
                '<div class="info">' + '<span><i class="red-star">*</i>荣誉描述:</span>' + '</div>' +
                '<div class = "file-panel" style = "height: 30px;" ><input class="inbiaoti check-input" type="text" value="" placeHolder="此处不可为空" maxlength="40"></div>' +
                '</div>'
            );
            $img = $li.find('img');
        }
        ...
        if (tempName == "cover-img") {
            ...
            if ($list.find('.file-item').length >= 15) {
                // $filePicker.hide();
    ...
    uploader.on('uploadSuccess', function (file, response) {
        ...
        $('#' + file.id).find('.nameval').val(response.data.url)
