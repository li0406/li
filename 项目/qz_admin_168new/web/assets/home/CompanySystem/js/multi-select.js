    ...
    MultiSelect.prototype = {
            ...
            if (ms.next('.ms-container').length === 0) {
                ...
                ms.find('optgroup, option').each(function () {
                    if ($(this).is('optgroup')) {
                        ...
                        var selectableLi = $('<li ' + attributes + '><span>' + $(this).text() +
                                '</span></li>'),
                        ...
                        selectedLi
                            ...
                            .attr("data-id", msId)
                            .attr("data-group", this.getAttribute("data-group"))
                            .hide();
                        ...
                        if (optgroupId) {
                            that.$selectableUl.children('#' + optgroupId + '-selectable').find('ul').first()
                                .append(selectableLi);
                            that.$selectionUl.children('#' + optgroupId + '-selection').find('ul').first()
                                .append(selectedLi);
                        } else {
                ...
                ms.on('keydown', function (e) {
                    if (ms.is(':focus')) {
                        var keyContainer = that.$selectableUl.hasClass('ms-focus') ? 'ms-selectable' :
                            'ms-selection';
        ...
        'select': function (value, method) {
            ...
            var that = this,
                ...
                selectables = this.$selectableUl.find('#' + msIds.join('-selectable, #') + '-selectable').filter(
                    ':not(.' + that.options.disabledClass + ')'),
        ...
        'deselect': function (value) {
            ...
            var that = this,
                ...
                selections = this.$selectionUl.find('#' + msIds.join('-selection, #') + '-selection').filter(
                    '.ms-selected'),
                options = ms.find('option').filter(function () {
                    return ($.inArray(this.value, value) > -1);
                });
        ...
        getValue: function (callback) {
            if (typeof callback == "function") {
                var val = this.$selectionUl.find(".ms-selected").map(function () {
    ...
    $.fn.multiSelect = function () {
        ...
        return this.each(function () {
            var $this = $(this),
                ...
                options = $.extend({}, $.fn.multiSelect.defaults, $this.data(), typeof option === 'object' &&
                    option);
            if (!data) {
