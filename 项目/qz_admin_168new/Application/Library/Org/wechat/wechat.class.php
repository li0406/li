...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function sendMassMessage($data){
        ...
        load('extend');
        Log::write(json_encode($result), LOG::WARN, LOG::FILE,
                             LOG_PATH . "wechat" . date("Y-m-d") . ".log");
        echo 2;
        if ($result)
		{
			$json = json_decode($result,true);
    ...
    public function sendBatchTemplateMessage($data,$codes){
        if (!$this->access_token && !$this->checkAuth()) return false;
        //批量处理 , 生成同样多的请求url,并格式化请求参数
        $urls = [];
        if(count($data) > 0){
            foreach ($data as $k=>$v){
                $data[$k] = self::json_encode($v);
                $urls[] = self::API_URL_PREFIX.self::TEMPLATE_SEND_URL.'access_token='.$this->access_token;
            }
        }
        $info = curl_post_batch($urls,$data);
        if(count($info) > 0){
            $returnData = [];
            $index = count($codes);
            foreach ($info as $k=>$v){
                $returnData[$index]['result'] = $v;
                $returnData[$index]['id'] = $codes[$index-1]['id'];
                $returnData[$index]['jc'] = $codes[$index-1]['jc'];
                $returnData[$index]['openid'] = $codes[$index-1]['openid'];
                $index--;
            }
            return $returnData;
        }
...
class TPWechat extends Wechat
{
    ...
    protected function getCache($cachename){
        //查询当前的token是否有效
        $Wechat = D("WechatToken")->getLastToken($this->appid);
        $time = time();
        $token = $Wechat["token"];
