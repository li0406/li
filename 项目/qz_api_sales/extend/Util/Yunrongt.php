...
class Yunrongt
{
	...
	{
		if (empty(config("app.yunrongt.user_name")) || empty(config("app.yunrongt.password"))) {
			throw new tException('短信服务用户名密码未设置好');
		}
	}
	...
	public function sendMessage($data)
	{
		...
		if (intval($result) < 1) {
			$reArr['errmsg'] = $this->errorMsg[intval($result)];
		} else {
			$reArr['errmsg'] = '';
		}
	...
	public function sendYxMessage($data)
	{
		...
		if (array_key_exists($result,$this->errorMsg)) {
	...
	{
		$data['username'] = isset($data['userName']) ?: config("app.yunrongt.user_name");

		if (isset($data['passWord'])) {
			$data['password'] = md5($data['username'] . md5($data['passWord']));
		} else {
			$data['password'] = md5($data['username'] . md5(config("app.yunrongt.password")));
		}
		$getparam = '?' . http_build_query($data);
	...
	{
		$data['username'] = isset($data['userName']) ?$data['userName']: config("app.yunrongtyx.user_name");
		$data['password'] = md5($data['username'] . md5(isset($data['passWord']) ? $data['passWord']: config("app.yunrongtyx.password")));
		// $data['body']     = rawurlencode($data['body']);
