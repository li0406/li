...
class Umeng {
	...
	CONST TYPE_ORDER = 1; // 订单提醒
	CONST TYPE_ROB_ORDER = 2; // 抢单提醒
	CONST TYPE_MSG = 3; // 消息中心提醒
	CONST TYPE_VISIT_OREDER = 4; // 回访单提醒
	...
	function sendAndroidListcast($tokenlist,$orderid,$type,$ticker = 'Android broadcast ticker',$title='中文的title',$text='Android broadcast text',$after_open='go_app',$notreadcount=0,$mipush=false,$mi_activity="") {
		try {

			$brocast = new \AndroidListcast();  //这个是发送类型吗  Listcast 表示列播
			$brocast->setAppMasterSecret($this->appMasterSecret);
			$brocast->setPredefinedKeyValue("appkey",           $this->appkey);
			$brocast->setPredefinedKeyValue("timestamp",        $this->timestamp);
			$brocast->setPredefinedKeyValue("device_tokens",	$tokenlist);
			...
			$brocast->setExtraField("orderid", $orderid);
            $brocast->setExtraField("type", $type);
            $brocast->setExtraField("badge", (int)$notreadcount);

            if($mipush == true){
                $brocast->setPredefinedKeyValue("mipush", true);
                $brocast->setPredefinedKeyValue("mi_activity", $mi_activity);
            }

			$getreturn = $brocast->send();
            $getreturn = json_decode($getreturn,true);
			if($getreturn['ret'] == 'SUCCESS'){
			    return true;
            }else{
			    return false;
            }
    ...
    function sendIOSListcast($device_tokens,$orderid,$type,$alert,$notreadcount=0) {
        try {
            $brocast = new \IOSListcast();
            $brocast->setAppMasterSecret($this->appMasterSecret);
            $brocast->setPredefinedKeyValue("appkey",           $this->appkey);
            $brocast->setPredefinedKeyValue("timestamp",        $this->timestamp);
            $brocast->setPredefinedKeyValue("device_tokens",$device_tokens);
            $brocast->setPredefinedKeyValue("description", $alert);
            $brocast->setPredefinedKeyValue("alert", $alert);
            $brocast->setPredefinedKeyValue("badge", (int)$notreadcount);
            ...
            $brocast->setCustomizedField("orderid", $orderid);
