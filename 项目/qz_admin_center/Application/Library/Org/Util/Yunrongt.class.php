...
EOF;
    ...
    */
    ...
    $reArr['errcode'] = $result; //errcode为0为发送成功
  ...
  public function curl_get($data) {
      ...
      $curl = curl_init();
      curl_setopt($curl, CURLOPT_URL, "http://101.201.238.246/MessageTransferWebAppJs/servlet/messageTransferServiceServletByXml"
                                       . $getparam);
    ...
    public function curl_get_new($data) {
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, "http://47.98.61.138:9001/smsSend.do"
            . $getparam);
        curl_setopt($curl, CURLOPT_HEADER, false);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_TIMEOUT, 3);
