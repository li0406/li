...
class UserModel extends Model{
    ...
    public function getUserInfoById($id = '',$cs=''){
        ...
        $buildSql = M("user")->where($map)->alias("a")
                             ...
                             ->field("a.id,a.on,a.cs,a.qc,a.jc,a.kouhao,a.logo,a.pv,a.video,a.qq,a.qq1,a.cal,a.cals,a.dz,a.tel,a.case_count as casecount,b.text as jianjie,b.jiawei,b.fuwu,b.fengge,b.quyu,b.fake,b.nickname,b.nickname1,b.hengfu,b.guimo,b.chengli,b.img_host,b.img,b.cooperate_mode")
                             ->buildSql();
        //2.查询出风格，区域，风格的信息
        $buildSql = M("user")->table($buildSql)->alias("t")
                             ...
                             ->join("LEFT JOIN qz_guimo as f on f.id = t.guimo")
                             ->field("t.id,t.on,t.chengli,t.cs,t.qc,t.jc,t.kouhao,t.img_host,t.img,t.logo,t.video,t.pv,t.jianjie,t.jiawei,t.fake,t.nickname,t.nickname1 ,t.hengfu,t.qq,t.qq1,t.cal,t.cals,t.dz,t.tel,t.casecount, GROUP_CONCAT(DISTINCT  c.qz_area ORDER BY c.qz_areaid) as area,GROUP_CONCAT(DISTINCT d.name order by d.px DESC) as fw,GROUP_CONCAT(DISTINCT e.name order by e.px ) as fg,f.name as gm,t.cooperate_mode")
                             ->group("t.id")->buildSql();
        //3.取出设计师的数量
        $buildSql = M("user")->table($buildSql)->alias("t1")
    ...
    public function getTeamDesignerList($comid='',$zw='',$zt ="",$pageIndex = 0,$pageCount=10)
    {
        ...
        //查询出设计师的简介信息
        return  M("user")->table($buildSql)->alias("t")
                             ->join("LEFT JOIN qz_user_des as b on t.id = b.userid and b.company_id=0")
   ...
   */
    ...
    public function getDesingerInfo($id='',$cs =''){
        ...
        $buildSql =  M("user")->where($map)->alias("a")
                 ...
                 ->join("LEFT JOIN qz_user_des as c on c.userid = a.id and c.company_id = 0")
     ...
     * 获取公司设计师信息
     * @param  string $id  [设计师编号]
     * @param  string $company_id [公司编号]
     * @return [type]      [description]
     */
    public function getNewDesingerInfo($id = '',$company_id)
    {
        $map = [
            "a.id" => ["EQ", $id],
            'a.position' => ['in', [2, 3, 4]],
            'a.state' => ['eq', 1],
            'c.company_id' => ['eq', $company_id],
        ];
        $buildSql = M("user_company_employee")->where($map)->alias("a")
            ->join("LEFT join qz_team as b on a.id = b.userid and b.zt = 2")
            ->join("LEFT JOIN qz_user_des as c on c.userid = a.id")
            ->join("left join qz_user as d on d.id = b.comid")
            ->join("left join qz_quyu as e on e.cid = d.cs")
            ->field("a.id as uid,a.nick_name name,c.*,a.logo,d.qc,d.id as comid,e.bm,b.zw")->buildSql();
        return M("user")->table($buildSql)->alias("t")
            ->join("left join qz_article as e on e.userid = t.userid")
            ->field("count(e.id) as acount,t.*")
            ->find();
    ...
    public function getCompanyEmployee($id){
        ...
        return M("user_company_employee")->where($map)->find();
    }
     ...
     * @return [type] [description]
     */
    public function editDesignerPv($id,$company_id){
        $where = [
            'userid'=>$id,
            'company_id'=>$company_id
        ];
        M("user_des")->where($where)->setInc('popularity',1);
    }
