...
class DiaryModel extends Model{
     ...
     * @return
     */
    public function getMobileIndexDiaryList($limit = 3)
    {
        $map = array(
            'stat' => 1,
        ...
        $build = M("diary_info")->field('id, user_id, title, diary_time, page_view, reply_count')->where($map)->order("diary_time desc")->limit($limit)->buildSql();
        return M()->table($build)->alias('d')
                                 ->field('d.*, u.nickname AS user_name, u.avatar AS user_logo')
                                 ->join('join qz_ucenter_profile u ON u.uuid = d.user_id')
                                 ->select();
    }
    ...
    public function get_one_diary_info($diary_id=0,$follow_diary=false,$isUser=false){
        ...
        $info =  M('diary_info')->where($map)->alias("a")
                 ...
                 ->join("LEFT JOIN qz_ucenter_profile as u on u.uuid = a.company_id")
                 ->join("LEFT JOIN qz_quyu as q on q.cid = u.city")
    ...
    public function get_all_diary_list_api($limit = '',$cid =''){
        ...
        if($count)
        {
            ...
            $buildSql = M("diary_info")->table($buildSql)->alias("t")
                                       ...
                                       ->buildSql();
            $list =  M("diary_info")->table($buildSql)->group("t1.parent_id")->alias("t1")->order("t1.parent_id desc,t1.diary_time DESC")->select();
            //dump(M()->getLastSql());
            import('Library.Org.Util.Fiftercontact');
            $filter = new \Fiftercontact();
            foreach ($list as $key => $v)
        ...
        return compact("list","pageTmp","totalpage");
    }
    ...
    public function getHotDiaryUser($limit=5,$week = true,$istop = true){
        ...
        $list = M('diary_info')->where($map)->alias("a")
                                   ->join("INNER JOIN qz_ucenter_profile as u on a.user_id = u.uuid")
                                   ->order("a.page_view desc")
                                   ->limit('0,'.$limit)
                                   ->field("a.id,a.user_id,a.title,a.add_time,a.mianji,a.fengge,a.huxing,u.name,u.avatar as logo")
                                   ->select();
        //dump(M()->getLastSql());
        return $list;
    }
    ...
    /**
     * 根据parent_id获取相关日记
     * @param  array $map           查询条件：id,parent_id
     * @return array|void $result
     */
    public function get_diarys_by_parent_id($map){
        $map['stat'] = 1;
        $result = M('diary_info')->where($map)->order("diary_time desc")->select();
        foreach($result as $k => $v){
            $result[$k]['img_list'] = $this->get_diary_img_by_id($v['id'],$limit);//获取图片列表
        }
        return $result;
    }
    ...
    /**
     * 根据名称获取相关装修公司
     * @param  strting $name           查询条件：公司名称/简称
     * @return array|void $result
     */
    public function getDiaryCompanyByName($name){
        if(!empty($name)){
            $where = [
                array('a.user'=>array('like','%'.$name.'%')),
                array('a.jc'=>array('like','%'.$name.'%')),
                '_logic'=>'OR'
            ];
            $result = M("user")->alias('a')
                            ->join("left join qz_quyu as q on q.cid = a.cs")
                            ->field('a.id,a.user,a.cs,q.cname,q.bm')
                            ->where($where)
                            ->limit('1')
                            ->select();
            return $result;
