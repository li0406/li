...
class OrdersModel extends Model{
    ...
    public function getOrderInfoById($id){
        ...
        return M("orders")->where($map)->alias("a")
                          ...
                          ->join("LEFT JOIN qz_province as province on province.qz_provinceid = q.uid")
                          ...
                          ->field("a.*,a.fengge as fg,province.qz_province provincename,b.qz_area,c.name as yusuan,d.name as hxname,f.name as fengge,g.name as fangshi,h.tel8 as tel8,q.cname")
                          ->find();
    }
    ...
    */
    ...
    public function getCityPrice($cs)
    {
        ...
        return M("order_city_info")->where($map)->field("half_price_min,half_price_max")->find();
    }

    /**
     * getXiaoQuByOrderId 根据订单id获取小区名称
     * @param $orderid
     */
    public function getXiaoQuByOrderId($orderid){
        if($orderid){
            $map['id'] = $orderid;
        }else{
            return '';
        }
        $return = M('Orders')->where($map)->getField('xiaoqu');
    ...
    /**
     * 获取开工大吉数据
     * @param int $cityId       获取城市的id
     * @param int $excludeCityId    排除城市的id
     * @param int $limit  获取数量
     * @return array
     */
    public function getPassTheaAuditOrder($cityId = 0, $excludeCityId = 0, $limit = 10)
    {
        $map = [
        ...
        if ($cityId > 0) {
            $map["o.cs"] = array("eq", $cityId);
        }

        // 排除掉的城市
        if ($excludeCityId > 0) {
            $map["o.cs"] = array("neq", $cityId);
        }


        $buildSql = M("orders")->alias("o")
            ->field("o.`name`,o.cs,u.jc,o.xiaoqu,o.qiandan_addtime,o.id,o.mianji")
            ->where($map)
            ->join("INNER JOIN qz_user as u on o.qiandan_companyid = u.id")
            ->join("INNER JOIN qz_user_company c ON c.userid = u.id")
            ->order("o.qiandan_addtime DESC")
            ->limit($limit)
            ->buildSql();

        $list = M()->table($buildSql)->alias("t")
            ->field("t.`name`,q.cname,t.jc,t.xiaoqu,t.mianji")
