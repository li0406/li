...
class QuyuModel extends Model{
    ...
    public function getCityById($cid)
    {
        ...
        if (!$cityInfo) {
            ...
            S("C:M:q:info:".$cid,$cityInfo,3600*6);
        }
        return $cityInfo;
    }
    ...
    */
    ...
    //
    public function getJiaJuOpenCityByid($cityid = 0){
        $where = [];
        $where['a.cid'] = $cityid;
        $where['a.is_open_city'] = 1;
        $citylist = M('jiaju_quyu')->alias('a')
            ->where($where)
            ->field('a.*,upper(left(a.px_abc,1)) AS first_abc')
            ->order('a.px,id,a.px_abc')
            ->find();
        return $citylist;
    }
