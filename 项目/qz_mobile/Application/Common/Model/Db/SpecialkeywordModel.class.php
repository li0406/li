...
class SpecialkeywordModel
{
     ...
     * @return [type]            [description]
     */
    public function getActriceBySpecialkeywordList($keyword_module,$name,$tempword, $pageIndex = 0, $pageCount = 12){
        if($keyword_module == 1){
            $map = "a.title like '%".$name."%'";
            $field = "a.id, a.title, a.subtitle as description, a.face, b.shortname,case when a.title like '%".$name."%' then 1 ";
    ...
    public function getSpecialkeywordByAbout($name,$tempword){
        $map = "name <> '".$name."' and (name like '%".$name."%'";
	 ...
	 * @return [type]            [description]
	 */
	public function getNewSpecialkeyword($map){
