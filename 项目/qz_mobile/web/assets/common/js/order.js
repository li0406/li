...
(function () {
    ...
    var prompt = {
        ...
        'tel':{
            'notempty':'您填写的手机号码为空 ^_^!',
            'ismobile':'请填写正确的手机号码 ^_^!'
        },
        'cs':{
            'notempty':'请选择您所在的城市 ≧▽≦'
        },
        ...
        'mianji':{
            'notempty':'请填写您的房屋面积 ^_^!',
            'isnumber':'请您填写真实的房屋面积!',
            'acreagerange':'房屋面积请输入1-1000之间的数字 ^_^!'
        },
        'xiaoqu':{
            'notempty':'请填写您的小区以便精准匹配 ^_^!'
        },
        'disclamer':{
            'noChecked':'请勾选我已阅读并同意齐装网的《免责声明》！'
        }
    };
    ...
    var order = function (params) {
        ...
        // 发送请求前置灰
        if (!params.submitBtn.className||!params.submitBtn.type) {
            console.error("未配置发单按钮信息！！")
            // return false
        }
        orderButtonChangeStatus(params.submitBtn.className, params.submitBtn.type)
        ...
        .done(function(data, status, xhr) {
            //请求成功回调函数
            orderButtonChangeStatus(params.submitBtn.className, params.submitBtn.type,"success")
